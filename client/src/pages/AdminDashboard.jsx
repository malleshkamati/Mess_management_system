    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="max-w-6xl mx-auto px-4 py-6">
                ...
                {activeTab === 'dashboard' && (
                    <div className="space-y-6">
                        ...
                        <div className="card p-6">
                            ...
                            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 max-w-lg mx-auto">
                                {/* Calendar Header */}
                                <div className="grid grid-cols-7 gap-1 mb-2">
                                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map(day => (
                                        <div key={day} className="text-center text-[9px] font-bold text-gray-400 uppercase tracking-tighter">
                                            {day}
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="grid grid-cols-7 gap-1">
                                    {(() => {
                                        ...
                                        for (let i = 0; i < firstDay; i++) {
                                            grid.push(<div key={`pad-${i}`} className="aspect-square bg-gray-100/30 rounded-md border border-dashed border-gray-200"></div>);
                                        }
                                        ...
                                        for (let d = 1; d <= daysInMonth; d++) {
                                            ...
                                            grid.push(
                                                <div key={d} className="aspect-square bg-white rounded-md border border-gray-100 p-0.5 flex flex-col gap-0.5 relative group bg-white shadow-sm hover:shadow-md transition-all">
                                                    <span className="text-[8px] font-bold text-gray-300 ml-0.5 leading-none">{d}</span>

                                                    <div className="flex flex-col gap-0.5 flex-1">
                                                        {['breakfast', 'lunch', 'dinner'].map(type => {
                                                            const meal = dayData.find(m => m.type === type);
                                                            const wastage = meal ? meal.wastagePercent : 0;
                                                            // Proportional color calculation: 120 (Green) to 0 (Red)
                                                            const hue = 120 - (wastage * 1.2);

                                                            return (
                                                                <div
                                                                    key={type}
                                                                    className={`flex-1 rounded-[1px] cursor-help transition-all hover:scale-110 relative group/cell ${!meal ? 'bg-gray-50' : ''}`}
                                                                    style={meal ? { background: `hsl(${hue}, 70%, 50%)` } : {}}
                                                                >
                                                                    {meal && (
                                                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-gray-900 text-white p-3 rounded-xl text-[10px] z-50 opacity-0 invisible group-hover/cell:opacity-100 group-hover/cell:visible transition-all shadow-xl pointer-events-none">
                                                                            <div className="font-bold border-b border-white/20 pb-1 mb-2 flex justify-between capitalize">
                                                                                <span>{type}</span>
                                                                                <span className="text-gray-400">{d} {now.toLocaleDateString('en-US', { month: 'short' })}</span>
                                                                            </div>
                                                                            <div className="space-y-1 text-left">
                                                                                <div className="flex justify-between">
                                                                                    <span>Prepared:</span>
                                                                                    <span className="font-bold">{meal.estimatedPrepared}</span>
                                                                                </div>
                                                                                <div className="flex justify-between text-red-300">
                                                                                    <span>Waste:</span>
                                                                                    <span className="font-bold">{meal.estimatedWastage} ({meal.wastagePercent}%)</span>
                                                                                </div>
                                                                            </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl shadow-sm">
                            ...
                            <div className="flex flex-1 w-full md:w-auto gap-3 items-center">
                                ...
                                <button onClick={() => setShowMealForm(true)} className="btn btn-primary whitespace-nowrap px-4 py-2 h-10">
                ...
                {activeTab === 'settings' && (
                    <div className="space-y-4">
                        <h2 className="text-lg font-bold text-gray-800">Meal Timing Settings</h2>
                        <p className="text-gray-500 text-sm">Configure default meal times and cancellation deadlines.</p>
                                    ...
                                    <div className="space-y-3">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                defaultValue={settings[type]?.mealTime || ''}
                                            />
                                        </div>
                                        <div>
                                            <label className="text-sm text-gray-500 flex items-center gap-1">
                                                <Calendar size={14} /> Cancel Cutoff
                                            </label>
                                            <input
                                                type="time"
                                                className="input mt-1"
                                                defaultValue={settings[type]?.cancelCutoff || ''}
                                            />
                                        </div>
                                    </div>
                                </div>
                        ...
                        <p className="text-xs text-gray-400">
                            Note: Changing settings here will apply to newly created meals. Existing meals keep their original timings.
                        </p>
